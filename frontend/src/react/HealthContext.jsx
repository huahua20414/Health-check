...
export function HealthProvider({ children }) {
  ...
  const actions = useMemo(() => ({
    ...
    updateAppointmentPayment: (appointment, paymentStatus) => action('appointment', paymentStatus === 'paid' ? '已模拟支付成功' : '已撤销模拟支付', async () => { await request(`/appointments/${appointment.id}/payment`, { method: 'PATCH', body: JSON.stringify({ paymentStatus }) }); await loaders.loadAppointmentsPage() }),
